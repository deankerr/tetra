...
| `docs/storage.md` | Store vocabulary and the current persistence, sync, deletion, and ownership model across web, CLI, worker, schemas, and tinydb.          |

## Package And App Contexts

| Path                              | Scope                                                                                                                                                                      |
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/core/CONTEXT.md`        | Core domain behavior: sessions, messages, runs, catalog refresh, tool execution, and shared behavior used by web and CLI. Run recovery belongs here when implemented.      |
| `packages/tinydb/CONTEXT.md`      | Typed TinyBase helper library design: zod-to-TinyBase schema generation, `db` collections/queries/values APIs, React hooks, escape hatches, and library-level constraints. |
...
- `packages/schemas` defines the shared `library` and `catalog` schemas; it does not own live store instances or their lifecycles.
- App composition roots create store instances and own surface-specific persistence and sync. `docs/storage.md` is the cross-context authority for those boundaries.
- `packages/tinydb` describes a reusable typed TinyBase helper library.
