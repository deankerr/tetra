...
- `src/components/ai-elements/markdown-components.tsx`: set fenced code block body background to the `canvas` token (`bg-canvas`) so the code content area is lifted off the near-black floor.
- `src/components/ui/sheet.tsx`: change `SheetContent` background from `bg-background` to the `canvas` token (`bg-canvas`); the near-black overlay resets elevation, so the sheet sits a step above the canvas floor.
...
- `src/components/ui/sidebar.tsx`: `sidebarMenuButtonVariants` — reclaim the menu-action gutter (Arc-style). At desktop width (`md:`) the `pr-8` reservation only applies while the row is hovered, focus-within, or has an open action, so idle titles use the full width; below `md:` the button stays always-reserved (`max-md:group-has-data-[sidebar=menu-action]`) for touch. Also dropped `padding` from the button `transition-[…]` so the width change is instant rather than an attention-drawing slide.
