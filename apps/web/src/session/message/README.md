...
`messageId` plus thread-derived display facts such as whether this is the current
thread leaf; nested slices can read TinyBase directly when they need lightweight,
reactive state.
...
they should sit behind a Tetra wrapper that maps sibling message ids to the selector
and persists no synchronized focus state.
