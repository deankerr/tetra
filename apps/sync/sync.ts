...
const server = createWsServer(
  wss,
  (pathId) => {
    const safeName = pathId.replaceAll(/[^a-zA-Z0-9-_]/g, '-') || 'default'
  ...
  // eslint-disable-next-line promise/prefer-await-to-callbacks -- createWsServer error handler, not a promise callback
  (error: unknown) => {
...
Bun.serve({
  fetch(req) {
    ...
    if (parts[0] === 'table' && tableId !== undefined) {
      if (rowId !== undefined) {
    ...
    if (parts[0] === 'messages' && tableId !== undefined) {
      const rows = filterRows(store, 'messages', 'sessionId', tableId)
    ...
    // GET /requests/:sessionId — requests for a session
    if (parts[0] === 'requests' && tableId !== undefined) {
      const rows = filterRows(store, 'requests', 'sessionId', tableId)
      const sorted = Object.entries(rows)
